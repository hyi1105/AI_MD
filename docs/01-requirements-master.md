...
> 更新：2026-07-19 | **F-01～F-10 為目錄工具唯一 ID 對照表**（平台本體見 STATUS）
...
|------|------|
| 前端 | HTML + CSS + JS（`web/`） |
| 列表資料 | `web/catalog.json`（`data.js` 負責載入） |
| 詳情資料 | `web/tools/*.md`（無檔時用 catalog 欄位） |
| 追蹤 v1 | localStorage |
| 追蹤 v2 | 登入同步（待定） |
| 運行 | **GitHub Pages**（`https://hyi1105.github.io/AI_MD/`） |
| 本機開發 | `web/start-local.ps1` |
| 維護 | 你 + Cursor AI；**預設只改 MD**，改程式／發布須明示 |

## 變更紀錄

| 日期 | 摘要 |
|------|------|
| 2026-07-19 | 列表來源改 catalog.json；詳情路徑改 web/tools；協作改文件優先 |
