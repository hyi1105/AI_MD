...
> 更新：2026-08-14（追加：個人理財／共同維護 Excel＋帳單丟 AI）
...
#### 2026-08-14 — 個人理財：共同維護試算表＋帳單丟 AI
- 狀態：waiting-owner  
- 來源：對話  
  - 「我想你幫我理財 我最近花太了 也想要投資 能怎麼做」  
  - 「我想要共同維護 excel 之類的 常常有帳單能直接丟給你」  
- 為什麼（Why）：花太多又想投資時，若沒有一份雙方都能改的帳本，現金流看不清；帳單若要自己再打字進表，很容易放棄。不做會繼續漏記、控不了支、也談不上投資。  
- 做什麼（What）：負責人與 AI **共同維護一份帳本**（Excel／Google 試算表／repo 內 CSV 擇一）；負責人**直接丟帳單**（截圖／PDF／拍照／文字），AI 辨識後寫入列（日期、商家、金額、分類、備註），並可回報週／月結與控支提醒。投資起手仍排在緊急金之後。  
- 怎麼做（How）：  
  - **首選協作帳本（最小、對齊你說的）**：  
    1. 定好欄位模板（日期／商家／金額／幣別／分類／來源帳單／備註）。  
    2. 帳本放哪：Google 試算表（分享連結）／本機 Excel 上傳對話／repo 內 `finance/*.csv`（待拍板）。  
    3. 丟帳單 → AI 抽出欄位 → 追加列 → 可請人確認再寫入。  
    4. 週結／月結：分類加總、異常大額、訂閱重複、距緊急金目標進度。  
  - **不做**券商下單；投資區先只做配置檢查清單／定期定額提醒。  
  - **隱私**：帳單常有卡號後四碼、地址、個資 → 寫入前遮敏、帳本勿公開 commit 到公開 repo（若用 repo 需 `.gitignore` 或私有）。  
  - 細節確認前：**只改 MD、不動 code、不上線**。  
- 優點（Pros）：對齊「共同維護＋帳單直接丟」；摩擦低、容易養成；帳本可長期帶走。  
- 缺點／風險（Cons）：OCR／辨識可能錯金額；個資外洩風險；公開 repo 不可放真實帳單；非投資顧問聲明仍要有。  
- 不做的替代方案：負責人自己記 Excel，偶爾貼月結文字給 AI 看；或只做本站記帳工具（無共同試算表）。  
- 完成後列入 checklist：若做成站內工具才登記（例：能上傳帳單並追加一筆）；若僅對話＋外部試算表協作，**不進 checklist**。  
- 待拍板：  
  1. 帳本放哪？**Google 試算表**／本機 Excel 傳對話／**repo 內 CSV**（私有）？  
  2. 寫入規則：AI 辨識後**先給你確認再寫**，還是**直接寫入再抽查**？  
  3. 分類清單要用哪套（必要／想要／負債／儲蓄，或更細：外食、交通、訂閱…）？  
  4. 幣別與地區（台幣／台灣為主？）  
  5. 月收、固定支出、存款／負債粗估；緊急金有無？  
  6. 「花太多」主要類別？投資偏好（保守／中庸／積極）？  
  7. 要不要順便做**本站上傳帳單頁**，還是先只靠 Cursor 對話丟檔就夠？  
