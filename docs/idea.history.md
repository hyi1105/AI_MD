...
## 2026-08-16 — 合作規則：不必確認細節，直接開發並發布

- 結果：done（規則入檔）  
- 來源：對話（「改規則 沒有要我確認的直接發佈」）  
- 摘要：拿掉「待確認細節／確認後才做」閘門；想法→idea→立刻開發並發布；只有明確說先別做／先討論／先不要上線才停；缺資訊用假設補齊。已更新 AGENT-MASTER、`.mdc`、docs／README／idea 模板。  

## 2026-08-15 — 整庫分類：獨立 idea＝資料夾＝首頁按鈕

- 結果：done  
- 來源：對話（隨時丟獨立 idea；首頁按鈕；資料夾獨立；請建議分類並整理）  
- 摘要：定兩類 `tool`／`demo`；新增 `web/apps.json`＋`home.js` 分組渲染；docs/README 寫整庫慣例；路徑維持扁平 `web/<slug>/` 不包 apps 層以免斷網址。  
...
- 來源：對話（試血緣→「可以 這樣的呈現方式我還願意閱讀完」）  
- 摘要：先「從哪來」再跟一條欄位走完；條件血緣與命名不一致要標出。  

## 2026-08-13 — 跑 .ps1 前也用說明法

- 結果：done  
- 來源：對話（以後叫我 run .ps 也要這樣介紹；理解後再決定是否 run）  
- 摘要：§7／說明法.mdc／口令卡加入「指令前先說明，同意後再跑」。  

## 2026-08-13 — 說明法可攜到別的 repo

- 結果：done  
- 來源：對話（「我也要用在別的repo」）  
- 摘要：新增可複製規則 `.cursor/rules/說明法.mdc`；也可貼 Cursor User Rules 讓所有專案生效。  

## 2026-08-13 — 說明法永遠套用（讀文件＋讀程式）

- 結果：done  
- 來源：對話（「非常好這是我要的」；問如何告訴每個新對話；未來讀文件與程式都要這樣）  
- 摘要：§7 定名「說明法」；`.mdc` alwaysApply；口令「用說明法」；含口令卡可貼 User Rules。  

## 2026-08-13 — 記憶法：路線＋錨點＋旁註（結構跟著出現）

- 結果：done（後併入說明法定稿）  
- 來源：對話（記路線；錨點；旁註）  
- 摘要：父層同行介紹＋子層旁註；一次 ≤5；空一行接續；讀碼左邊貼近原文、一層一層；禁止房間比喻。  
