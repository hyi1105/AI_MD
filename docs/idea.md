...
- 狀態：open（規則已寫入 AGENT-MASTER／`.mdc`；本則待下一輪實際依新流程跑通後結案）  
- 來源：對話（原文：「規則開成 跟我確認好細節後 都直接開發並發布」）  
- 為什麼（Why）：舊規則要分開說「改程式」「發布」，多一層口令；負責人要的是**細節對齊後就一次做完上線**。  
- 做什麼（What）：想法階段只改 MD；AI 列出待確認細節；負責人確認後，AI 直接開發並發布，不必再喊口令。  
- 怎麼做（How）：已更新 `prompts/AGENT-MASTER.md`、`md-first-workflow.mdc`、`theme.mdc`、`catalog-updates.mdc`、`00-theme`、`03-checklist`、`README`。  
- 優點（Pros）：少來回、節奏快；仍用「確認細節」當安全閘。  
- 缺點／風險（Cons）：若「確認」語義模糊可能誤觸發；AI 應只接受明確確認，細節未列清不開幹。  
- 不做的替代方案：維持雙口令「改程式／發布」。  
- 完成後列入 checklist：合作方式＝確認細節後直接開發並發布（已登記；本則結案時機＝下一功能依新流程發布成功後）。  
- 待拍板：無（規則已開）  
...
- 狀態：waiting-owner（**等確認下列細節**；一確認就依新規則直接開發並發布）  
...
- 做什麼（What）：系統地圖看起來像在拼一塊系統：每個節點是一塊圖塊，層級可嵌套；切換 SQL／流程圖／PA／Shared Enclosure 時，**同一塊換皮不換位**。  
- 怎麼做（How）：改 `web/system-map/`；少長文、多圖與短標；切換視角 2～3 個有意動畫；續用簽核示範。  
- 優點（Pros）：對齊胃口；空間記憶；統一場景更像換皮玩具。  
- 缺點／風險（Cons）：造型成本；小螢幕密度。  
- 不做的替代方案：只加圖示；或靜態總圖 PNG。  
- 完成後列入 checklist：系統地圖節點以拼圖／圖塊呈現，且切換至少兩視角時同一節點可辨為同一塊。  
- **待確認細節（請回「確認」或改條件）**：  
  1. 風格＝**積木／磁貼圖塊**（不做真拼圖缺角；較好 RWD）。  
  2. 範圍＝只改 `web/system-map/` 視覺與互動，不擴新系統。  
  3. 四視角換皮：流程＝箭頭軌道、SQL＝表欄位、PA＝觸發－動作積木、Enclosure＝空間席位。  
  4. 確認後依新規則：**直接開發並發布**。  
...
#### 系統地圖再強化：拼圖式圖像化
- 狀態：waiting-owner（詳見上方；等確認 4 點細節）  
- 待確認：積木磁貼；只改 system-map；四視角換皮；確認後直接開發發布  
