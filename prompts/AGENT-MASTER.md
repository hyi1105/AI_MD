...
> 目的：負責人**無法靠一次講完再對照來記憶**；要用**故事一個一個串起來**。  
> 禁止：同一則把骨架＋細節＋多題對照一次倒完（那不是一層一層）。

### 負責人記憶法（必守）

- **故事串連**：每一則只講「一章」——一個場景、一件事、一個為什麼。  
- **章與章相扣**：下一章必須接上一章的結尾（像連續劇），不要突然換地圖重講。  
- **一章一個檢查點**：最多 **1** 題選擇題（或一句「要不要進下一章」）；不要章末丟 3 題總複習。  
- **等他接**：沒說「繼續／下一章／OK」就不要自動開下一章。  
- **不能記清單牆**：大樹狀圖、多區塊並列、一次講完再對照＝失敗寫法。

### 何時套用

凡回覆「解法、架構、程式碼、怎麼改、為什麼這樣做、repo／資料怎麼維護」時套用。  
...
```text
① 接上集（一句）
② 本章故事（一個比喻／一個場景，含一個為什麼）
③ 本章只點到「這章需要的」路徑或名字（不要整庫目錄）
④ 一題檢查點 或 「要不要下一章？」
⑤ 程式碼本體（通常不下；真要改檔才附，且仍最後、仍短）
```
...
|------|----------------|
| 最怕開頭大段程式 | 程式永遠最後；可省略 |
| 曾選「同則展到檔案層」 | **被故事法覆蓋**：一章只帶本章路徑；整庫分多章 |
| 選擇題先觀察 | 改為**每章最多 1 題**；禁止章末 3 題總複習 |
| 「2、3 像一次講完才對照；1 的量也太多」 | 單則必須更短；分章等他接 |
| 「無法記憶，得用故事一個一個串」 | 正式記憶法＝故事串連 |

### 不做

- 不做站上產品頁（產品化另開 idea）。  
- 不要長文黑箱、不要目錄牆、不要一次講完再三題對照。  
- 不要一開始就貼大段程式。
