...
> 更新：2026-08-15（追加：規則精簡／省 Token）

## 2026-08-15 — 規則精簡：問一句也不要整包燒 Token
- 狀態：waiting-owner
- 來源：對話（「感覺這樣很耗 Token」「有時候我只是想問個問題」）
- 為什麼（Why）：`.cursor/rules` 多份 `alwaysApply`（含說明法全文範本）每輪對話都會塞進上下文；只問一句也付「整包規則」成本，浪費且拖慢。
- 做什麼（What）：平常問答／閒聊只載入極短指針；要開工、讀碼、跑指令、用說明法時才載入細節規則。
- 怎麼做（How）（待拍板後改檔）：
  1. **合併＋瘦身**：`theme`／`md-first`／`catalog` 收成 1 份短 `alwaysApply`（約十幾行指針，不貼範本）。
  2. **說明法改為按需**：`說明法.mdc` 取消 `alwaysApply`（改 `agent_requestable` 或口令「用說明法」才讀）；範本只留在 `AGENT-MASTER` §7。
  3. **主控不重複**：`.mdc` 只寫「見 AGENT-MASTER §x」，禁止與主控全文重複貼。
  4.（可選）口令「只問問題」＝本輪不主動展開結構樹／不寫 idea，除非你要整理。
- 優點（Pros）：日常問答明顯省 Token；規則仍可一鍵叫回；與「idea／history 拆檔省成本」同一精神。
- 缺點／風險（Cons）：新對話若沒載入說明法，AI 可能忘記結構樹節奏；需靠短指針＋口令補回；瘦身時要避免漏硬規則（未確認不動 code）。
- 不做的替代方案：維持現狀；或只刪說明法範本、其餘 alwaysApply 不動。
- 完成後列入 checklist：開新對話問一句無關問題時，規則側只應見短指針、不見說明法長範本；說「用說明法」仍能正確呈現
- 待拍板：
  1. 要做哪一檔？A 合併三份短指針＋說明法按需／B 只瘦說明法／C 先不動
  2. 「只問問題」要不要當成正式口令？
  3. 硬規則（未確認只改 MD、目錄只碰 catalog、繁中）是否必須永遠 alwaysApply？
