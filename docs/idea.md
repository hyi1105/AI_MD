...
## 2026-08-13 — 填寫欄位＝對話框（欄位即 LINE 格）

- 狀態：open
- 來源：對話（原文關鍵句：「我想要那些填寫欄位就是對話框」）
- 為什麼（Why）：現在假畫面是傳統 `input`／`select` 紙本格，跟右側聊天室分開，填寫不像「對話」、也難對齊產品「對話室＝LINE 格、格可限定欄位」。不做會繼續像一般表單，簽核討論與填值割裂。
- 做什麼（What）：使用者在簽核畫面裡，**每一個要填的欄位本身就是一則／一串對話框**（類 LINE 氣泡）：點欄位或在對話串回覆即可填入；簽核人看得到對應欄位的對話內容（隱私欄仍依身分隱藏整格）。紙本總覽可保留，但主填寫路徑是對話框，不是傳統表單控件。
- 怎麼做（How）：
  1. 改 `web/approval/` Demo：欄位區改為「每欄一則對話串／氣泡卡」（標籤＝欄位顯示名，內容＝值；可回覆＝編輯）。
  2. Schema（`web/schema/form-schema.example.json`）標註欄位與對話格對應（例如 `ui: dialog_box`、可選 `bound_chat_thread`）。
  3. 右側對話室可改為「依欄位篩選的對話串」或與欄位對話框合併成同一主錨（待拍板）。
  4. 對齊 Approval 願景：對話室 LINE 格＋紙本引擎；本階段仍是靜態／本機 Demo，不接 SharePoint。
- 優點（Pros）：填寫＝溝通同一套 UI；符合「類 LINE」記憶；簽核討論自然落在欄位上；利於之後「格可限定欄位」隱私。
- 缺點／風險（Cons）：版面變密、長文欄難排；下拉／數字／是非在氣泡裡的控件要另設計；與現有 Teams＋紙本並排 Demo 改動大；易和「整張單一個聊天室」搞混。
- 不做的替代方案：維持傳統欄位，只在右側聊天用 `#假別` 這種標籤綁欄位（較小改動）。
- 完成後列入 checklist：簽核 Demo 中每個可填欄位以對話框（氣泡／對話串）呈現與填寫；隱私欄在簽核人視角仍整格消失。
- 待拍板：
  1. 主畫面要以「每欄一對話框」取代紙本 grid，還是紙本＋對話框並存？
  2. 下拉／數字／人員欄在對話框裡用什麼控件（氣泡內選項／快選按鈕／另開小面板）？
  3. 右側整串聊天室要保留、收合，還是併進欄位對話？
