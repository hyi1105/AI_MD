...
> 更新：2026-08-14（追加：Cursor 意圖理解準確率基準）
...
#### Cursor 意圖理解準確率（相對 Gemini／ChatGPT 約好上 90%）
- 狀態：waiting-owner  
- 來源：對話（原文：「cursor誤會我們要的意思跟Gemini char GPT好上90%」→ 訂正：「不是，是 cursor **不誤會**的機率比其他人高 90%；cursor 理解力很正確」）  
- 為什麼（Why）：先前容易把「Cursor vs 其他家」聽成「Cursor 比較會誤會」；若不訂正，後續產品／合作設計會朝錯誤方向走。負責人實際體感是：**Cursor 較少誤會意圖，理解力明顯較正確**（相對 Gemini／ChatGPT 約好上 90%）。  
- 做什麼（What）：把「Cursor 級少誤會」記成**意圖理解基準**；本 repo 的合作規則（說明法、AGENT-MASTER、idea 流轉）繼續以 Cursor 為主戰場；自家 AI 工具（說明法／AI Doc）若要比「懂不懂人話」，以 Cursor 的準確率當標竿，而不是向較易誤會的對話型產品對齊。  
- 怎麼做（How）：先只改 MD 定錨（本則）；確認後再決定是否寫進 `AGENT-MASTER`／`00-theme` 一句「意圖理解基準＝Cursor」、或做成產品內評測項。不動 code、不上線。  
- 優點（Pros）：避免聽反；合作與產品設計有明確「誰懂我們」的基準。  
- 缺點／風險（Cons）：「好上 90%」是體感比例，非正式評測；若寫進對外文案需再量化或改成定性描述。  
- 不做的替代方案：只在對話裡記住、不入檔（易再聽反）。  
- 完成後列入 checklist：合作文件有「意圖理解以 Cursor 少誤會為基準」可查核句（若負責人確認要寫進規則）  
- 待拍板：① 只當內部備註，還是寫進 `AGENT-MASTER`／theme 一句？②「好上 90%」要保留體感數字，還是改成「明顯較少誤會」？③ 是否要拿來當說明法／AI Doc 的評測標竿？  
