...
#### 負責人記憶法寫進合作規則
- 狀態：waiting-owner  
- 來源：對話（「你知道我的記憶法嗎」；同時要 repo 結構地圖；選擇題說還不知道、需要多久嘗試）  
- 為什麼（Why）：AI 若不依你的記憶法組資訊，解法地圖仍難進長期記憶，只能當下點頭。Repo 裡目前**沒有**寫明你的記憶法。  
- 做什麼（What）：你用自己的話說出記憶法 → AI 整理進 `AGENT-MASTER`（或獨立短節）→ 之後解法／repo 說明都用同一套記法呈現。  
- 怎麼做（How）：先只改 MD／規則；不改產品頁。可與 §7 方案地圖合併或並列。  
- 優點（Pros）：說明方式對齊你怎麼記；少「聽懂當下、過兩天忘」。  
- 缺點／風險（Cons）：記錯你的方法會誤導；需你校正一輪。  
- 不做的替代方案：繼續只用方案地圖（骨架→為什麼→檔案層＋3 題），不另記記憶法。  
- 完成後列入 checklist：合作規則可查到「依負責人記憶法組織說明」  
- 待拍板：請你用自己的話描述記憶法（步驟／比喻／禁忌）；是否與 §7 合併  
