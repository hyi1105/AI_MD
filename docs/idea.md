...
## 2026-08-13 — 簽核＝像打 LINE，但有必填格式；送出＝submit／approve

- 狀態：open（待「改程式」）
- 來源：對話（「填寫欄位就是對話框」→「主要像打 LINE，有被要求填寫格式；送出＝submit／approve」→ 拍板：格式卡／聊天≠卡片／駁回退回也要）
- 為什麼（Why）：現在 Demo 是傳統表單格＋旁邊聊天，填寫與對話割裂。實際要的是**以 LINE 式對話為主路徑**，同時系統仍強制「該填的格式／欄位」；送出那一下要對應簽核動作（申請人 submit、簽核人 approve），不是一般聊天「傳訊息」。不做會繼續像 OA 表單，或變成無結構的閒聊。
- 做什麼（What）：
  1. 主畫面像打 LINE：氣泡對話、可回覆、可看對方訊息（**一般聊天就是聊天**）。
  2. **格式卡是另一個東西**：像平日一邊聊天、一手拿著要填的文件——卡片≠聊天氣泡；必填欄、型態、選項在格式卡上完成，沒填完不能算送出成功。
  3. 底部主按鈕語意＝**送出**：申請人＝`submit`（送簽）；簽核人＝`approve`（核准）。
  4. **駁回／退回**也做成對話裡的系統訊息＋按鈕（不當成普通聊天字）。
- 怎麼做（How）：
  1. 改 `web/approval/`：主錨＝LINE 風對話室；**格式卡**獨立區塊／可釘選面板（非嵌成聊天氣泡），與訊息流並列——「手上文件 vs 對話」兩套 UI。
  2. Schema 定義必填格式（假別、天數、代理人…）；值寫在格式卡，對話室只顯示一般聊天＋系統事件（送出／核准／駁回／退回）。
  3. 送出鈕：依角色「送出申請」→ `submit`、「核准」→ `approve`；未完成必填則擋下並在格式卡標缺漏。
  4. 駁回／退回：對話出現系統卡＋按鈕；點了留下系統訊息（含理由輸入若 Demo 需要）。
  5. 隱私欄只在申請人格式卡出現；簽核人卡片無此格。
  6. 本階段 Demo／本機狀態即可，不接 SharePoint。
- 優點（Pros）：聊天與文件心智分離清楚；仍像打 LINE；submit／approve／駁回／退回語意清楚。
- 缺點／風險（Cons）：要同時做好對話室＋格式卡兩套版面（尤其手機）；系統訊息與一般氣泡要明顯區隔；與現有 Teams＋紙本並排 Demo 改動大。
- 不做的替代方案：維持傳統表單，聊天只當留言板（不符合「主要像打 LINE」）。
- 完成後列入 checklist：簽核 Demo 以 LINE 對話為主、格式卡為必填文件；兩者視覺分離；申請人送出＝submit、簽核人＝approve；駁回／退回為對話內系統訊息＋按鈕；未填完必填不可送出。
- 待拍板：無（1 格式卡／2 聊天≠卡片／3 駁回退回要 — 已定）。說「改程式」即依此改 `web/approval/`。
