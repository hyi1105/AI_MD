...
#### 方案地圖：用圖與步驟讓非專家能判斷
- 狀態：open  
- 來源：對話（原文關鍵：給程式／解法時變成可快速理解的 map；flowchart／methods；最大問題是沒知識只能同意 AI）  
- 為什麼（Why）：負責人沒有足夠專業知識來「審核」AI 的說法，只能點頭同意 → 決策風險高、信任成本高。不做的話，每次解法都是黑箱文字牆，更難追問與否決。  
- 做什麼（What）：之後凡給「程式碼、解法、架構建議」，預設先給一張**可掃一眼的地圖**（流程圖／步驟樹／決策點），再附精簡說明；地圖上標出「你可拍板的點」與「我假設了什麼」。目標是：你不必懂實作細節，也能判斷方向對不對。  
- 怎麼做（How）：  
  1. **合作規則**（優先、零工程）：寫進 `prompts/AGENT-MASTER.md`——回覆解法時固定結構：地圖 → 一句結論 → 假設／風險 → 待你拍板 →（必要時才）程式。  
  2. **圖的格式**：對話裡用 Mermaid／ASCII／條列樹；複雜場景可對齊既有「系統地圖」風格（積木／視角）。  
  3. **判斷輔助**（可選產品化）：之後若要做成站上工具，可做「解法說明卡」或延伸 system-map；**本則先以合作規則為主**，產品頁面另案。  
- 優點（Pros）：降低「只能同意」；拍板點清楚；與既有 idea→確認→開發流程一致；幾乎零成本可先上路。  
- 缺點／風險（Cons）：地圖若過細會變第二種黑箱；寫規則後需每次遵守；產品化會與 system-map 重疊需分界。  
- 不做的替代方案：維持長文說明；或只在你要求「畫圖」時才畫。  
- 完成後列入 checklist：AI 給解法時預設有地圖＋拍板點（合作規則可查）；若做成產品頁則另列「能開啟解法地圖頁」。  
- 待拍板：  
  1. 範圍只要「對話規則」還是也要「站上產品」？  
  2. 地圖預設粒度：粗（3～7 塊）還是細（含檔案／函式）？  
  3. 要不要強制每則解法都列出「反對／替代」讓你比較容易否決？  
