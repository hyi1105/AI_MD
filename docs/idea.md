...
- 狀態：open  
- 來源：對話（原文關鍵：「layer by layer」「兩個結構展開」「SQL／Shared Enclosure／流程圖／Power Automate 都看到類似場景」）；後續「沒想法、先用自行」→ AI 暫定下列決策  
...
- 怎麼做（How）：  
  ...
  - **暫定落地路徑（AI 自行，負責人可改）**：先寫方法論＋一本靜態示範知識書（MD），**不做產品 UI**；驗證「四視角對齊」後，再說「改程式」。示範系統用站上已有脈絡的**簽核／申請**。  
- 優點（Pros）：學習有地圖可循；跨工具溝通有共同座標；與主題「知識書／累積專業」高度契合；可複用 AI Doc 多模式預覽概念；先 MD 成本低、可改。  
- 缺點／風險（Cons）：統一模型若設計過重會變成抽象稅；不同領域硬套同一層級可能失真；暫定「簽核」若與負責人腦中 Shared Enclosure 定義不合需再換示範。  
- 不做的替代方案：直接做可切換視角 UI（成本高、未驗證格式）；或只寫抽象方法論不帶示範（難感覺「類似場景」）。  
- 完成後列入 checklist：能打開同一系統地圖並在至少兩種視角（例如流程圖與 SQL）之間切換，且層級節點仍對齊可辨識。（產品化後才登記；本階段僅 MD 示範時不進 checklist。）  
- 暫定決策（負責人說「先用自行」；隨時可改）：  
  1. **先方法論＋靜態 MD 示範**，不做產品 UI。  
  2. **第一本示範＝簽核／申請**（申請單 ↔ 流程 ↔ 資料表 ↔ 自動化通知／Shared Enclosure 共置附件）。第一層暫定兩主幹：**流程控**（誰批、到哪）＋**資料存**（單據與狀態）。  
  3. **視角四個起跳**：SQL／Shared Enclosure／流程圖／Power Automate。  
- 待拍板：無（暫定如上；負責人不同意再說改）  
...
- 怎麼做（How）：先 MD 方法論＋簽核示範；統一節點卡＋四視角投影；產品 UI 之後  
- 優點（Pros）：有地圖、可對齊理解；先 MD 可改  
- 缺點／風險（Cons）：模型過重或示範選錯領域  
- 不做的替代方案：直接做 UI；或只寫抽象方法論  
- 完成後列入 checklist：至少兩視角切換且層級對齊（產品化後）  
- 暫定決策：方法論＋MD；示範＝簽核；四視角；待拍板＝無  
