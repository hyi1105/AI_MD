...
#### 說明法擴充：非線性流程／血緣／其他類型
- 狀態：open  
- 來源：對話（「還有什麼類型可嘗試？例如簽核流程圖非線性、資料庫欄位血緣圖」）  
- 為什麼（Why）：說明法已定稿於檔案樹／函式／if-for／.ps1；非線性圖（流程分叉、欄位血緣）用純直樹可能不夠，需試幾種「同一套節奏、不同錨點」的變體，否則讀圖時又變一次倒完。  
- 做什麼（What）：負責人可逐類試用；每類仍遵守一次 ≤5、同行介紹、一層一層；試完說「這是我要的」再寫進 §7 定稿。  
- 怎麼做（How）：先只改對話呈現／規則 MD；不必做站上產品。可對現有 `web/approval/`、`web/system-map/`、schema 當素材試。  
- 優點（Pros）：同一記憶節奏覆蓋更多知識型；與簽核／地圖示範素材現成。  
- 缺點／風險（Cons）：非線性硬塞成一棵樹可能歪；變體太多難記。  
- 不做的替代方案：非線性仍用 system-map 畫面，說明法只用於檔案與程式。  
- 完成後列入 checklist：§7 列出已定稿類型清單（含非線性／血緣若確認）  
- 待拍板：先試哪 1～2 類（見下方候選）  
