...
1. **先給路線**：少數**真實路徑／檔名**當錨點串成一條路（例如 `web/ → web/catalog.json`），結構一眼在旁邊。  
2. **每個錨點一句話**：這是什麼／為什麼在這裡（極短）。  
3. **錨點必須是實際檔案結構**（資料夾、檔名）；禁止用「你說話→確認」這類虛流程冒充結構。  
4. **一則只走一小段路**：不要整庫一張牆；下一段等他說「繼續」。  
5. **檢查點最多 1 個**（或只問要不要下一段）；禁止章末三題總複習。  
6. **程式碼最後、可省略**；絕不要開頭大段程式。

### 一則的樣子（範本）

```text
路線：path/A → path/B → path/C
...
| 「要實際檔案結構，不是虛的」 | 錨點＝真實 path；禁流程比喻冒充結構 |
| 舊「同則檔案牆／三題／長故事／虛流程路線」 | 作廢 |
