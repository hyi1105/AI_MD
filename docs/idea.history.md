...
## 2026-08-13 — 記憶法：故事一個一個串（覆蓋舊方案地圖量）

- 結果：done（已寫入 AGENT-MASTER §7 並發布）  
- 來源：對話（看到問題：2、3 像一次講完才對照，不是一層一層；1 的量也太多；「無法記憶，得用故事一個一個串起來」）  
- 摘要：正式記憶法＝故事串連；每則一章；章末最多 1 檢查點；等他說繼續才下一章；禁止目錄牆＋三題總複習。舊「同則展到檔案層／章末三題」作廢。  

## 2026-08-13 — 方案地圖說明（對話規則 A）

- 結果：done（後被同日「故事串連」修訂覆蓋細節）  
- 來源：對話（解法要成 map；確認：只要 A／一層一層骨架再到細節、為什麼引導像導師；結尾 3 則選擇題）  
- 摘要：初版 §7；後依選擇題與記憶法回饋改寫。  
