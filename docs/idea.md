...
#### 說明法擴充：非線性流程／血緣／其他類型
- 狀態：open（**血緣已定稿**；非線性等其餘類型待試）  
- 來源：對話（可試類型；選血緣；「可以 這樣的呈現方式我還願意閱讀完」）  
- 為什麼（Why）：說明法已定稿於檔案樹／函式／if-for／.ps1；非線性圖等仍待試。  
- 做什麼（What）：血緣已可寫進 §7；其餘類型逐類試用後再說「這是我要的」。  
- 怎麼做（How）：先只改對話呈現／規則 MD；素材可用 approval／schema。  
- 優點（Pros）：同一節奏覆蓋更多知識型。  
- 缺點／風險（Cons）：非線性硬塞成樹可能歪。  
- 不做的替代方案：非線性仍用 system-map 畫面。  
- 完成後列入 checklist：§7 已含血緣；其餘類型確認後再補  
- 待拍板：下一步是否試「簽核非線性流程」  
