...
- 狀態：building
- 來源：對話（「幫我做一個 Hello World 的首頁，並提供連結」）
- 為什麼（Why）：需要一個最簡單、可立刻打開的首頁入口，確認站點與部署路徑可用；沒有清楚首頁時，訪客不知道從哪裡開始。
- 做什麼（What）：首頁顯示品牌 SEED 與「Hello World」問候；提供進入 AI 查詢／AI Doc 的連結。
- 怎麼做（How）：`web/index.html` 改為 Hello World 落地頁；既有 AI 查詢頁移到 `web/query.html`，並更新導覽連結。
- 優點（Pros）：入口清楚、可立刻給連結預覽；不破壞既有查詢功能。
- 缺點／風險（Cons）：首頁從工具目錄改為問候頁，舊書籤若預期直接搜尋需多點一次。
- 不做的替代方案：只在現有首頁加一行 Hello World（較亂）。
- 完成後列入 checklist：首頁可見 Hello World，且可連到 AI 查詢／AI Doc
- 待拍板：無
