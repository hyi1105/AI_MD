...
## 2026-08-10 — 功能棋盤 Portal 首頁

- 狀態：open  
- 來源：對話（「把所有獨立功能都分開, 首頁為可以選擇各功能的portal按鈕網站(每一個功能等於棋盤)」）  
- 為什麼（Why）：現在首頁是 Hello World 落地頁，功能入口只有兩個 CTA；各工具頁頂部又用 chip 互相切換，獨立功能邊界模糊。使用者應先進「總入口」選棋盤，再專心使用單一功能；新功能（書架等）也不該硬塞進既有頁的 nav。  
- 做什麼（What）：首頁變成 **Portal**：一排／一格可點的功能按鈕；每一個獨立功能＝一張「棋盤」（獨立頁或獨立路由，可完整使用該功能）。進入棋盤後以該功能為主，不再把其他功能混成同一套主導航。  
- 怎麼做（How）：
  1. **盤點棋盤（現況可獨立者）**：至少 `AI 查詢`（`query.html`＋詳情 `tool.html`）、`AI Doc`（`aidoc/`）；未來構想中的「知識書架」等新能力也以新棋盤掛上 Portal，不併入舊頁。  
  2. **改首頁 `web/index.html`**：以 Portal 按鈕網格取代（或大幅收斂）現有 Hello CTA；品牌 SEED 仍為第一視窗主訊號；每個按鈕進對應棋盤。  
  3. **拆開交叉入口**：弱化／移除工具頁頂部「AI 查詢 ↔ AI Doc」chip 互切（或改為「回 Portal」）；各棋盤只保留回首頁／本棋盤內導覽。  
  4. **可選**：用一份輕量棋盤清單（如 `web/portal.json` 或寫死在首頁）登記名稱、說明、href；目錄資料仍只靠 `catalog.json`，Portal 清單≠工具目錄。  
  5. 觸發詞：**「改程式」**才動 code；**「發布」**才上線並改 checklist。  
- 優點（Pros）：邊界清楚、首頁可擴充新棋盤、各功能可獨立演進、符合「工具分開、主題統一」的 Theme。  
- 缺點／風險（Cons）：多一次點選才進工具；舊書籤／習慣從 query 頂部跳 AI Doc 的路徑改變；需同步改 checklist「入口與品牌」條目。  
- 不做的替代方案：維持現有 Hello＋雙 CTA，只在文案寫「選工具」；或只加第三顆按鈕、不拆 nav。  
- 完成後列入 checklist：
  - 首頁為 Portal，以按鈕進入各獨立功能棋盤  
  - 每個已上線獨立功能各有一張棋盤入口  
  - 棋盤內不以主導航混切其他獨立功能（回 Portal 即可）  
- 待拍板：
  1. 「棋盤」要不要在 UI 上真的用這個詞（還是對內概念、畫面上寫「工具／功能」）？  
  2. 第一版棋盤清單是否只含 **AI 查詢**、**AI Doc**（書架等仍構想中不掛假入口）？  
  3. Hello World 問候要保留在 Portal 上，還是改成純選棋盤？  
...
#### 功能棋盤 Portal 首頁
- 狀態：open（詳見上方 2026-08-10 完整條目）  
- 摘要：首頁＝Portal 按鈕選棋盤；獨立功能分開；各棋盤不混主導航  
