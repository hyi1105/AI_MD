...
## 2026-08-11 — 刪除所有資料
- 狀態：waiting-owner
- 來源：對話（原文：「我想刪除所有資料」；Cloud Agent 任務名「所有資料刪除」）
- 為什麼（Why）：負責人想清空現有資料，避免舊內容干擾重做或重置產品狀態。不做則舊 catalog／本機狀態／構想匣會一直留下。
- 做什麼（What）：依拍板範圍，清除選定的資料層（本機使用者資料、倉庫內資料檔、或構想／歷史文件），讓產品回到乾淨起點。
- 怎麼做（How）：**先拍板範圍，再說「改程式」或「改目錄」才動手**。可對應資料層如下：
  1. **瀏覽器本機資料**（使用者端）：`ai_md_theme`、追蹤關鍵字、AI Doc 本機文稿／額度／SEED 索引等 `localStorage` — 可加「清除本機資料」按鈕，或文件說明如何手動清。
  2. **站上工具目錄**：`web/catalog.json`（約 300 筆）— 需負責人明確說「改目錄」才可清空或縮成空陣列／最小範例。
  3. **構想與歷史**：`docs/idea.md` 進行中條目、`docs/idea.history.md` 已結案 — 僅在負責人確認後搬空或歸檔刪除。
  4. **不建議刪**：`docs/00-theme.md`、`docs/03-checklist.md`、`docs/ai-doc.md`、程式與合作規則（除非要整站廢止）。
- 優點（Pros）：乾淨重啟；減少誤用舊資料；構想匣可重新聚焦。
- 缺點／風險（Cons）：不可逆（尤其 catalog 與 history）；線上 AI 查詢會變空；checklist 若連帶刪會失去查核基準；誤刪主題／規格會破壞合作流程。
- 不做的替代方案：只清本機 localStorage；或只把 catalog 縮成少數示範筆；或只把舊構想移到 `idea.history` 而不刪內容。
- 完成後列入 checklist：依實際做成項目補「能清除／已重置某某資料」查核句（未做成前不登記）
- 待拍板：
  1. 「所有資料」要清哪幾層？（本機／catalog／idea／history／其他）
  2. catalog 清空後，AI 查詢要顯示空狀態，還是留少數示範工具？
  3. 是否保留 theme／checklist／ai-doc 規格？
  4. 確認後請說「改目錄」或「改程式」（依範圍）再執行；「發布」才上線。
